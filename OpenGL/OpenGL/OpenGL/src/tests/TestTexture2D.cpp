#include "TestTexture2D.h"

test::TestTexture2D::TestTexture2D()
	:m_texturePathA("res/textures/zzz.jpg"),m_texturePathB("res/textures/godot.png")
{
	m_translationA = glm::vec3(0.0f, 0.0f, 0.0f);
	m_texturePtrA = new Texture(m_texturePathA.c_str());
	
	m_texturePtrA->Bind(0);
	
	m_ibPtr = new IndexBuffer(m_indices, 12);
	m_vbPtr=new VertexBuffer(m_positions, 8 * (2+4) * sizeof(float));
	m_layout.Push<float>(2);
	m_layout.Push<float>(4);
	m_va.AddBuffer(*m_vbPtr, m_layout);
	m_shaderPtr = new Shader("res/shaders/Basic.shader");
	
	
	m_proj = glm::ortho(0.0f, 960.0f, 0.0f, 640.0f, -1.0f, 1.0f);
	m_view = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, 0.0f));
	m_model = glm::translate(glm::mat4(1.0f), m_translationA);
	m_mvp = m_proj * m_view * m_model;
	
}

test::TestTexture2D::~TestTexture2D()
{
	m_texturePtrA->UnBind();
	
	m_va.UnBind();
	m_vbPtr->Unbind();
	m_ibPtr->Unbind();
	m_shaderPtr->UnBind();
	delete m_texturePtrA;
	
	delete m_ibPtr;
	delete m_vbPtr;
	delete m_shaderPtr;

}

void test::TestTexture2D::OnUpdate(float deltaTime)
{
	m_model = glm::translate(glm::mat4(1.0f), m_translationA);
	m_mvp = m_proj * m_view * m_model;
}

void test::TestTexture2D::OnRender()
{
	m_shaderPtr->Bind();
	m_shaderPtr->SetUniformMat4f("u_MVP", m_mvp);
	m_renderer.Draw(m_va, *m_ibPtr, *m_shaderPtr);
}

void test::TestTexture2D::OnImGuiRender()
{
	ImGui::SliderFloat3("Translation", &m_translationA.x, 0.0f, 960.0f);
}
